...
with (df, {
    ...
    plot(nDate, Sub_metering_1, type='l', col="black", xlab = "", ylab='Energy Sub Metering', xaxt='n')
    axis.POSIXct(side=1, x=df$nDate, format="%a", labels = TRUE)
    lines(df$nDate, df$Sub_metering_2, col="red")
    lines(df$nDate, df$Sub_metering_3, col="blue")
    legend ("topright", lty=1, col=c("black", "red", "blue"), bty = "n", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
    
    plot(nDate, Global_reactive_power, type='l', xlab = "datetime", ylab='Global_reactive_power', xaxt='n')
    axis.POSIXct(side=1, x=df$nDate, format="%a", labels = TRUE)
